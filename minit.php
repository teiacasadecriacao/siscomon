<?php
    ...
    { // utilizado em ver-pauta e em pautas-atuais
        $adata2 = explode("-",$adata);
        $adata3=date("d/m/Y",mktime(0,0,0,$adata2[1],$adata2[2]+$ndias, $adata2[0]));
        return $adata3;
    }
    ...
    function estado_pauta($data1)
    {
        ...
        $date1Int = mktime(0,0,0,$dateArr[1],$dateArr[2]+28,$dateArr[0]) ;
        $res=$date1Int -$data2;
        if($res>0)
            return "votacao";
    ...
    $conecta = mysql_connect("localhost","root", "foobar" ) or die (mysql_error());
    $selectdb = mysql_select_db("agora_commons",$conecta) or die ("base de dados nao localizada");
